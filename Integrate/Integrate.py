...
from Algorithms.DesignToolAlgorithmV1.EXTRAS.joinBlocks import jointBlock
from Algorithms.DesignToolAlgorithmV1.BoundaryConditions.FormBoundaryConditionInformation import FormBoundaryConditionInformation
class Integrate():
    def __init__(self, mesh, cfl_flag, tCurrent, currentStep) -> None:
        ...
        #for cell in self.mesh.cellArray:
            #print("p", cell.flowState.fs["p"], ", T", cell.flowState.fs["T"], ", rho", cell.flowState.fs["rho"], ", vel_x", cell.flowState.fs["vel_x"], ", a", cell.flowState.fs["a"])
            #print("Mass:", cell.conservedProperties["mass"], "xMom:", cell.conservedProperties["xMom"], "Energy:", cell.conservedProperties["energy"])
        ...
        #print("This is the post-convectino properties")
        for ind, cell in enumerate(self.mesh.cellArray):
            cell.update_properties()
            #print("index: ", ind, "p", cell.flowState.fs["p"], ", T", cell.flowState.fs["T"], ", rho", cell.flowState.fs["rho"], ", vel_x", cell.flowState.fs["vel_x"], ", a", cell.flowState.fs["a"])
